...
const MaduraiDist = () => {
  ...
  const DISTRICT_ID = 2; // Madurai ID
  ...
  if (!selected || places.length === 0) {
    return <div className="text-center py-20">No places found</div>;
  }
  ...
  return (
    <>
      ...
      <section className="py-16 px-4">
        <h2 className="text-3xl font-bold text-center mb-10">More Places in Madurai</h2>
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {places.map((place) => (
            <div
              key={place.id}
              onClick={() => navigate(`/place/${place.id}`)}
              className="rounded-xl overflow-hidden shadow-lg cursor-pointer hover:shadow-2xl transition"
            >
              <img src={place.hero_image} alt={place.name} className="w-full h-48 object-cover" />
              <div className="p-4">
